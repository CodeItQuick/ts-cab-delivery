...
async function cabDropOffCustomer() {
    const availableCab = await cabsRepository.findFirst(
        "TransportingCustomer", "No cab transporting a customer");
    const firstAvailableCustomer = await customerRepository.findFirst(
        "CustomerAssignCab", "No customers available to drop off");
...
async function customerCancelledRide() {
    const firstAvailableCustomer = await customerRepository.findFirst(
        "InitialCustomerCall", "No customers have called in");
